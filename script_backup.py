...
# training & validation
ROUNDS = 50 #50
SAMPLE_SIZES = np.array([0.05, 0.1, 0.25, 0.5, 1.0]) # np.linspace(0.05, 0.5, 2, endpoint=True) #1, 20
TRAIN_SIZE = np.array([0.3, 0.6, 0.9]) # np.linspace(0.1, 0.6, 2, endpoint=False) #1, 2
...
FS_CV_SPLIT_SIZE = np.array([2 , 4, 8]) # np.linspace(2, 10, 2, endpoint=True).astype(int) #5
...
PAR_SPLIT_SIZE = np.array([2 , 4, 8]) # np.linspace(2, 10, 2, endpoint=True).astype(int)
par_grid = {'svm': {'C': np.logspace(0.1, 20.0, num=5, base=2), 'gamma': np.logspace(4, -20.0, num=5, base=2)},
        ...
        while j < 1:
            if tries >= 1:
                entry = pd.DataFrame({'model': model,
                          ...
                          'mean_nan_performance': np.NaN,
                          'median_nan_performance': np.NaN,
                          'max_nan_performance': np.NaN,
                          'min_nan_performance': np.NaN}, index=[0])
                entry.to_csv('output_entries2', mode='a', index=False, header=False)
                return entry
            ...
            try:
                ...
                if validation_type == 'ts':
                    ...
                    X_data_without_nan_sel = X_data_without_nan.loc[:, feature_selection_mask]
                    ...
                    cv_res_nan = cross_validate(model_object, X_data_without_nan, y_data_without_nan,
                                           cv=cv,
                                           scoring=performance_metric)['test_score'].mean()
                    performance.append(cv_res)
                    nan_performance.append(cv_res_nan)
...
if __name__ == '__main__':
    ...
    output = pd.DataFrame(columns=['model', 'sample_size', 'sample_selectors', 'sample_size_selected', 'feature_selector',
                                   ...
                                   'max_performance', 'min_performance', 'mean_nan_performance', 'median_nan_performance',
    ...
    output.to_csv('output2', index=False)
