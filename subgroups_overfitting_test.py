...
# training & validation
ROUNDS = 1
SAMPLE_SIZES = np.array([0.05, 0.07, 0.1, 0.2, 0.5, 0.75, 1.0])
...
def do_calc(subgroup, model, main_metric, sample_size, feature_selector, feature_selection_frac, validation_type, par_split_size, cv_split_size, train_size):
    ...
    entries = pd.DataFrame({'subgroup': subgroup,
                            'model': model,
                           'main_metric': main_metric,
                           'sample_size': sample_size,
                           'feature_selector': feature_selector,
...
if __name__ == '__main__':
    ...
    reference = data.groupby(['target', 'age_group'], group_keys=False).apply(lambda x: x.sample(frac=0.2))

    reference.drop('age_group', axis=1, inplace=True)
                                ...
                                for par_split_size in PAR_SPLIT_SIZE:
                                    ...
                                    for cv_split_size in cv_split_size_list:
                                        for train_size in train_size_list:
                                            result.append(do_calc.remote(subgroup, model, main_metric, sample_size, feature_selector, feature_selection_frac,
                                                                         validation_type,
