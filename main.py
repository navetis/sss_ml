...
# todos
# daten initial analysieren, replacement argumentieren (evtl. variieren)
# run mit raw data
# run mit bereinigten daten (hier evtl noch korrelation einbauen)
# run mit untergruppe (geschlecht, und alter wären nice)
# evtl. non stratified sampling
...
# training & validation
ROUNDS = 50
SAMPLE_SIZES = np.linspace(0.03, 1, 50, endpoint=True)
TRAIN_SIZE = np.linspace(0.5, 1, 5, endpoint=False)
VALIDATION_TYPES = ['ts', 'all_nested', 'all_kfold', 'fs_nested_pt_kfold', 'fs_kfold_pt_nested']  # pt: parameter tuning
FS_CV_SPLIT_SIZE = np.linspace(2, 20, 10, endpoint=True).astype(int)
MODELS = ['svm', 'knn', 'naive_bayes', 'random_forest', 'decision_tree', 'logistic_regression']
PERFORMANCE_METRICS = ['accuracy', 'balanced_accuracy', 'top_k_accuracy', 'average_precision',
                       'neg_brier_score']  # use sklearn scoring parameters
FEATURE_SELECTOR = ['rfe', 'sequential']
FEATURE_SELECTION_FRAC = np.linspace(0.1, 1, 7, endpoint=True)  # relevant for rfe and sequential

# parameter ranges for models
PAR_SPLIT_SIZE = np.linspace(2, 20, 10, endpoint=True).astype(int)
par_grid = {'svm': {'C': np.logspace(0.1, 20.0, num=20, base=2), 'gamma': np.logspace(-0.1, -20.0, num=20, base=2)},
...
def tune_parameters(estimator, features, target, param_grid, split_size):
    ...
    clf = GridSearchCV(estimator=estimator, param_grid=param_grid, cv=cross_val)  # define a model with parameter tuning
...
if __name__ == '__main__':
    ...
    # print(data_without_nan.describe())
    # print(data_without_nan.columns)
    ...
    #                                'max_performance', 'min_performance', 'validation_type', 'par_split_size',
    #                                'fs_cv_split_size', 'mean_nan_performance', 'median_nan_performance',
    #                                'max_nan_performance', 'min_nan_performance'])
    #
    # for model in MODELS:
    ...
    #                                             X_data_without_nan_sel = X_data_without_nan.loc[:,
    ...
    #                                                 model_object.fit(X_train_sel, y_train)
    #                                                 predictions = model_object.predict(X_test_sel)
    #                                                 without_nan_predictions = model_object.predict(X_data_without_nan)
    #
    #                                                 inner_perf.append(
    #                                                     measure_performance(performance_metric, predictions, y_test))
    ...
    #
    #                                             performance.append(
    #                                                 cross_validate(model_object, X_sel, y,
    #                                                                cv=StratifiedKFold(n_splits=fs_cv_split_size,
    #                                                                                   shuffle=True),
    #                                                                scoring=performance_metric)['test_score'].mean())
    #                                             nan_performance.append(
    #                                                 cross_validate(model_object, X_data_without_nan, y_data_without_nan,
    #                                                                cv=StratifiedKFold(n_splits=fs_cv_split_size,
    #                                                                                   shuffle=True),
    #                                                                scoring=performance_metric)['test_score'].mean())
    ...
    #                                                 model_object.fit(X_train_sel, y_train)
    #                                                 predictions = model_object.predict(X_test_sel)
    #                                                 without_nan_predictions = model_object.predict(
    #                                                     X_data_without_nan_sel)
    #
    #                                                 inner_perf.append(
    #                                                     measure_performance(performance_metric, predictions, y_test))
    #                                                 inner_nan_perf.append(measure_performance(performance_metric,
    ...
    #                                                           'num_feature_sel': int(
    #                                                               feature_selection_frac * len(X.columns)),
