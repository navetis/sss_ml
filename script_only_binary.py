    ...
    for i in range(ROUNDS):
        ...
        while j < 1:
            if tries >= 1:
                ...
                entry.to_csv('output_entries_new_binary', mode='a', index=False, header=False)
                return entry
...
if __name__ == '__main__':
    ...
    data.replace({'target': [1,2,3,4]}, value=1, inplace=True)
    ...
    output.to_csv('output_new_binary', index=False)
